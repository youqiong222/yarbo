...
defineCustomElement(
  'deferred-media1',
  () =>
    class DeferredMedia1 extends HTMLElement {
      constructor() {
        super();
      ...
      pauseVideo() {
        ...
        if (tagName === 'VIDEO') {
          ...
          // Require links to be carried enablejsapi=1
          if (deferredElement.classList.contains('js-youtube')) {
            deferredElement.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');
          } else if (deferredElement.classList.contains('js-vimeo')) {
            deferredElement.contentWindow.postMessage('{"method":"pause"}', '*');
          }
        }
      }
    },
...
class HeaderDrawer extends MenuDrawer {
  ...
  openMenuDrawer(summaryElement) {
    ...
    document.documentElement.dispatchEvent(new CustomEvent('mobile-nav:open'));
  }
  ...
  closeMenuDrawer(event, elementToFocus) {
    ...
    document.documentElement.dispatchEvent(new CustomEvent('mobile-nav:close'));
  }
...
class ybAnchor extends HTMLElement {
  constructor() {
    super();
  }
  ...
  connectedCallback() {
    this.addEventListener("click", (e) => {
      ...
      if (!anchorId) {
        console.warn("[yb-anchor] 缺少 data-anchor 属性");
        return;
      }
      const anchorElement = document.getElementById(`shopify-section-${anchorId}`);
      this.dispatchEvent(new CustomEvent("yb-click", {
        bubbles: true,
        composed: true,
        detail: { anchorId }
      }));
      if (anchorElement) {
        window.scrollTo({
          top: anchorElement.offsetTop - 120,
          behavior: "smooth"
        });
      } else {
        console.warn(`[yb-anchor] 未找到对应的锚点元素: shopify-section-${anchorId}`);
      }
    });
  }
}

customElements.define("yb-anchor", ybAnchor);

class BuyButtonsNew extends HTMLElement {
  connectedCallback() {
    const notifyBtn = this.querySelector('[data-notify-me-trigger]');
    if (notifyBtn) {
      this._notifyHandler = (e) => {
        e.preventDefault();
        window.dispatchEvent(
          new CustomEvent('notify-me:open', {
            detail: {
              productTitle: notifyBtn.dataset.productTitle || '',
              variantTitle: notifyBtn.dataset.variantTitle || '',
              productImage: notifyBtn.dataset.productImage || '',
              variantId: notifyBtn.dataset.variantId || '',
              productId: notifyBtn.dataset.productId || '',
            },
          }),
        );
      };
      this._notifyBtn = notifyBtn;
      notifyBtn.addEventListener('click', this._notifyHandler);
    }

    const wrapper = this.querySelector('.product-form__error-message-wrapper');
    const msgEl = this.querySelector('.product-form__error-message');
    if (wrapper && msgEl) {
      this._observer = new MutationObserver(() => {
        ...
        if (window.__SL_toast__) {
          window.__SL_toast__.error({ message: text });
        }
        wrapper.setAttribute('hidden', '');
      });
      this._observer.observe(wrapper, { attributes: true, attributeFilter: ['hidden'], childList: true, subtree: true, characterData: true });
    }
  }

  disconnectedCallback() {
    if (this._notifyBtn && this._notifyHandler) {
      this._notifyBtn.removeEventListener('click', this._notifyHandler);
    }
    if (this._observer) {
...
customElements.define('buy-buttons-new', BuyButtonsNew);
